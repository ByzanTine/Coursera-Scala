...
class WikipediaApiTest extends FunSuite {
  ...
  test("concatRecovered given test case 1") {
    val request = Observable.just(1, 2, 3, 4, 5).concatRecovered(num => if (num != 4) Observable.just(num) else Observable.error(new Exception))
    val expectedString = "List(Success(1), Success(2), Success(3), Failure(new Exception), Success(5))"
    val actual = request.toBlocking.toList.toString
    println(actual)
    assert(actual == expectedString, actual)
  }
