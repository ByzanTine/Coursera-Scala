...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs map {s:String => s.replaceAll(" ", "_")}
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs map {t:T => Success(t)} onErrorReturn {t:Throwable => Failure(t)}
