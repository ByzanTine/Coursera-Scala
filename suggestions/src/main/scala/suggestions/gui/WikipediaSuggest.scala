...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = searchTerms map {
      s:String =>  
        Await.result(wikipediaSuggestion(s), Duration.Inf)
    } recovered
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => 
      x match {
        case Success(s) => log(s); suggestionList.listData = s
    ...
    // TO IMPLEMENT
    val selections: Observable[String] =  button.clicks.filter { x => !suggestionList.selection.items.isEmpty }.map { x => suggestionList.selection.items.head }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections map 
    {
      x => Await.result(wikipediaPage(x),  Duration.Inf) 
    } recovered
