...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    /** Returns a stream of text field values entered in the given text field.
      ...
      */
    def textValues: Observable[String] = Observable.create { subsrciber => {
        val subscription = Subscription()
        field subscribe {
          case ValueChanged(tf) =>
            subsrciber.onNext(tf.text)
            if (subscription.isUnsubscribed)
              subsrciber.onCompleted()
        }
        subscription
      }
    }
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.create { subsrciber => {
        val subscription = Subscription()
        button subscribe {
          case ButtonClicked(bc) =>
            subsrciber.onNext(button)
            if (subscription.isUnsubscribed)
              subsrciber.onCompleted()
        }
        subscription
