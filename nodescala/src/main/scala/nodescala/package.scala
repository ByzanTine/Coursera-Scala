  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      Future.firstCompletedOf(fs) onComplete {
        case Success(x) => p.complete(Try(x))
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      new CancellationTokenSource {
        ...
        p.tryCompleteWith(f(cancellationToken))
        def unsubscribe() {
          p.trySuccess(())
        }
      }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try {
        ...
        case _:Throwable => throw new java.util.NoSuchElementException
      } 
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete {
        case Success(_) => p.complete(Try(cont(f)))
        case Failure(exp) => p.failure(exp) 
      }
      p.future
    }
